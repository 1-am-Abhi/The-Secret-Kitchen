...
/**
 * Routes generated by the file conventions in src/app. Note there is no `.png`
 * extension — `opengraph-image.tsx` is served at `/opengraph-image`, and
 * requesting `/opengraph-image.png` 404s.
 */
...
export function buildMetadata({
  ...
  path?: string;
  keywords?: string[];
  /** Overrides the generated brand card. Omit to use the default. */
  image?: string;
  noIndex?: boolean;
}): Metadata {
  ...
  return {
    ...
    openGraph: {
      ...
      title: fullTitle,
      description,
      // Only set explicitly when overridden. Left undefined, Next injects the
      // generated card from src/app/opengraph-image.tsx with a cache-busting
      // hash — better than us hardcoding a path that can drift.
